function wp_action  (data, svg_area, silent) {
    ...
    var csize = data.random;
    var no_label = false;
    var type;
    if (data.event == "motion") {
        type = 'anon';
    } else if (data.event == "temperature")  {
        type = 'bot';
    } else {
        type = 'user';
    }
    ...
    var abs_size = Math.abs(data.random);
    size = Math.max(Math.sqrt(abs_size) * scale_factor, 3);

    Math.seedrandom(data.event)
    var x = Math.random() * (width - data.random) + size;
...
function newuser_action(data, lid, svg_area) {
    play_random_swell();
    var messages = ['Welcome to ' + data.user + ', IoT newest user!',
                    'IoT has a new user, ' + data.user + '! Welcome!',
                    'Welcome, ' + data.user + ' has joined Munich IoT!'];
